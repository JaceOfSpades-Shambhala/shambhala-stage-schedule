...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v51**.
...
Every site release bumps ONE version number everywhere (v51 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=50/?v=51/g; s/<!-- v50 -->/<!-- v51 -->/' index.html
sed -i 's/?v=50/?v=51/g' app.js styles.css manifest.webmanifest
sed -i 's/stage-schedule-v50/stage-schedule-v51/; s/?v=50/?v=51/g' sw.js
```
...
Latest: v51 fixes stage-switch jitter by giving the panel's stage mark a fixed 150×64 box, only rebuilding the mark image when the stage actually changes (not on every live-status tick), and pre-decoding the other stage marks after startup. v50 moved the selected-day label out of the live panel's stage mark header and into the top of the set-times card (between hairline rules), so the day stays visible while scrolling the list. v49 relocated the stage-title poster into a compact mark header at the top of the live panel (so the set list stays above the fold when switching stages/days), restyled the selected stage/day pills as accent-tinted glass, and added an explicit empty-state message for stage/day combos with no listed sets. v48 added copy-only browser/PWA Hexlace connection codes and private cross-context synchronization while retaining the audit-safe schedule, undo, accessibility, offline caching, and Worker fixes documented above.
