...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v66**.
...
Every site release bumps ONE version number everywhere (v66 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=65/?v=66/g; s/<!-- v65 -->/<!-- v66 -->/' index.html
sed -i 's/?v=65/?v=66/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v65/stage-schedule-v66/; s/?v=65/?v=66/g' sw.js
```
...
- `GET/PUT /profiles/:profileId/owl-admin-traits` + member/admin bearer + `X-Profile-Key` → read/write bounded overrides for that private profile's Owl. Saved overrides republish with that Owl and pass through the renderer's validated override path. The route keeps its old name for storage compatibility.
