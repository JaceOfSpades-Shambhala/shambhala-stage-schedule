...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v68**.
...
Every site release bumps ONE version number everywhere (v68 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=67/?v=68/g; s/<!-- v67 -->/<!-- v68 -->/' index.html
sed -i 's/?v=67/?v=68/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v67/stage-schedule-v68/; s/?v=67/?v=68/g' sw.js
```
...
- `POST /lists/:readId/trade` `{targetReadId, targetTapToken}` with `X-Write-Key` → records one owner's validated physical-tag tap for 15 minutes. A trade is matched only by reciprocal owner intent. `GET /lists/:readId/trade` polls state, `POST .../trade/confirm` requires both owners to confirm before ownership/profile credentials swap, and `POST .../trade/cancel` abandons that side. Each ordinary Owl remains on its physical Hexlace throughout; trading is never an Owl-optional action. A V3 Camp Hexadecibel Owl on either side is rejected before a trade can match.
...
- `GET/PUT /profiles/:profileId/owl-admin-traits` + member/admin bearer + `X-Profile-Key` → read/write bounded overrides for that private profile's Owl. Saved overrides republish with that Owl. V1/V2 retain their camp-only freestyle behavior; V3 uses its own UV catalogue, mandatory Vortex, hero floor/cap, support cap, and 12-point budget, surfacing override violations in the preview instead of throwing. The route keeps its old name for storage compatibility.
...
- Terminology is strict: **Hexlace** means the physical NFC tag; **Hex Owl** means the generated identity image. The v1 and v2 renderers remain frozen and byte-stable by seed/version; v2 stays the ordinary assignment grammar and the unversioned API default. Stored Owls always render through their recorded version. Ordinary V2 generation uses Common/Rare/Legendary tiers, exactly one hero on Rare/Legendary, shuffled Festival Prism bands, aperture-matched eye art, and a permanently retired accessory category. V3 is separate and provenance-only: six weighted UV palettes, the mandatory four-ring Hexadecibel Vortex, one guaranteed hero with a 12% second-hero chance, at most two supports, a 12-point budget, and no trading. Only a direct claim of a camp-provisioned physical Hexlace can assign or promote V3; QR pairing, ordinary seeds, shared links, and QR codes cannot. Future art changes must add another renderer version instead of altering v1, v2, or v3.
