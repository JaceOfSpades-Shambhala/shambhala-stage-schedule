...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v67**.
...
Every site release bumps ONE version number everywhere (v67 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=66/?v=67/g; s/<!-- v66 -->/<!-- v67 -->/' index.html
sed -i 's/?v=66/?v=67/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v66/stage-schedule-v67/; s/?v=66/?v=67/g' sw.js
```
...
- `GET/PUT /profiles/:profileId/owl-admin-traits` + member/admin bearer + `X-Profile-Key` → read/write bounded overrides for that private profile's Owl. Saved overrides republish with that Owl and pass through the renderer's camp-only freestyle path, which accepts any combination of enabled traits. The route keeps its old name for storage compatibility.
...
- Terminology is strict: **Hexlace** means the physical NFC tag; **Hex Owl** means the generated identity image. The v1 renderer remains frozen and byte-stable by seed/version; v2 is the current assignment grammar. Both reuse the exact traced path from `hex-owl-base.svg`, and stored Owls always render through their recorded version. Ordinary V2 generation uses Common/Rare/Legendary tiers, exactly one hero on Rare/Legendary, shuffled Festival Prism bands, aperture-matched eye art, and a permanently retired accessory category. Camp member/admin custom rendering explicitly bypasses those rarity, weight, and combination rules while still requiring enabled catalogue values. Blacklight is catalogue-only (`campOnly`, disabled) until a provenance-aware mint path is specified. Future art changes must add another renderer version instead of altering v1 or v2.
