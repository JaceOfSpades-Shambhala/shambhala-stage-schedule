...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v64**.
...
Every site release bumps ONE version number everywhere (v64 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=63/?v=64/g; s/<!-- v63 -->/<!-- v64 -->/' index.html
sed -i 's/?v=63/?v=64/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v63/stage-schedule-v64/; s/?v=63/?v=64/g' sw.js
```
...
- `POST /camp/pairings` + admin bearer → creates a hashed, one-use, 10-minute access-only pairing code; `POST /camp/pairings/redeem` registers the receiving device's generated bearer key without touching its existing Hexlace identity or local profile data. A retry from the same device key is idempotent; a second device is rejected.
