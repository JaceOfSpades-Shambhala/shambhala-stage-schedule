...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v65**.
...
Every site release bumps ONE version number everywhere (v65 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=64/?v=65/g; s/<!-- v64 -->/<!-- v65 -->/' index.html
sed -i 's/?v=64/?v=65/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v64/stage-schedule-v65/; s/?v=64/?v=65/g' sw.js
```
...
- `POST /lists` `{name, sets, ping, physical:false, profileId?, profileKey?}` → private identity/profile metadata and an Owl once the name + saved-set eligibility rule is met; with admin bearer access plus `claimable:true, campRole:"member"|"admin"` → `{readId, claimToken, tapToken, campGrantToken}` (no write key stored — unwritable until claimed)
...
- `GET/PUT /profiles/:profileId/owl-admin-traits` + admin bearer + `X-Profile-Key` → read/write bounded admin-only Owl overrides. Saved overrides republish with that Owl and pass through the renderer's validated override path.
