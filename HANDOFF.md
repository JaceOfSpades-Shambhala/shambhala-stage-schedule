...
8. **One-time physical URL rewrite:** legacy physical Hexlaces contain only `?f=<readId>`. They still share set lists, release, and retain ownership, but tap-only Hexadex collection requires rewriting once with the new `&tap=<token>` URL from **Write my tag**. Do not put tap tokens in ordinary QR/share links.
...
- Global Owl numbers are allocated once per profile by a dedicated low-frequency Durable Object; the random 128-bit seed is generated server-side and stored with renderer version and season. A display-name change never affects the seed. Release retains the Owl in the profile and reclaim restores it; a user's own Owl otherwise never changes. Trading between two physical Hexlaces was removed in v75 — exchanging tags now means each side releases and the other retaps the newly unclaimed Hexlace.
