...
Everything needed to continue this project from any computer. Written 2026-07-05, current release **v71**.
...
Every site release bumps ONE version number everywhere (v71 at the time of writing). The pieces that must stay in sync:
...
```bash
sed -i 's/?v=70/?v=71/g; s/<!-- v70 -->/<!-- v71 -->/' index.html
sed -i 's/?v=70/?v=71/g' app.js styles.css manifest.webmanifest hex-owl.js hex-owl-playground.html
sed -i 's/stage-schedule-v70/stage-schedule-v71/; s/?v=70/?v=71/g' sw.js
```
...
- **Identity/collection:** `shambhala-2026-hexlace-identity` (`{readId, writeKey, name, profileId?, profileKey?, owl?, tapToken?, isPhysical?, pendingClaim?, claimScannedAt?, silentClaim?, dirty?, lastPublished?}`), `shambhala-2026-camp-access` (local bearer key plus confirmed role/readId), `shambhala-hex-owl-profile` (private profile credentials plus current Owl), `shambhala-hexadex-cache` / `shambhala-hexadex-pending`, and `shambhala-2026-hexlaces-collected` (array).
...
- `POST /lists` `{name, sets, ping, physical:false, profileId?, profileKey?}` → private identity/profile metadata and an Owl once the name + saved-set eligibility rule is met; with admin bearer access plus `claimable:true` → a regular claimable tag, or add optional `campRole:"member"|"admin"` for a privileged tag with a `campGrantToken` (no write key stored — unwritable until claimed)
...
- `POST /lists/:readId/claim` `{claimToken, writeKey, scannedAt, campGrantToken?, campAccessKey?}` → registers the CLAIMER's locally-generated keys when this is the earliest recorded scan; returns `{ok:true, accepted:false}` for later scans without changing ownership. Redeeming a valid grant from a directly provisioned physical camp Hexlace assigns the hidden Camp Hexadecibel rarity on the unified V4 Owl identity and attaches the same Owl to the tag. Access-only QR redemption grants editor eligibility without changing the recipient's Owl until they explicitly save a camp-tier choice. A per-Hexlace Durable Object serializes claims and honours earlier-scan takeovers for **seven days after the first successful claim**; after that, ownership locks permanently.
...
- `POST /lists/:readId/trade` `{targetReadId, targetTapToken}` with `X-Write-Key` → records one owner's validated physical-tag tap for 15 minutes. A trade is matched only by reciprocal owner intent. `GET /lists/:readId/trade` polls state, `POST .../trade/confirm` requires both owners to confirm before ownership/profile credentials swap, and `POST .../trade/cancel` abandons that side. Each ordinary Owl remains on its physical Hexlace throughout; trading is never an Owl-optional action. A V4 Owl carrying the Camp Hexadecibel tier on either side is rejected before a trade can match.
...
- `GET/PUT /profiles/:profileId/owl-admin-traits` + member/admin bearer + `X-Profile-Key` → read/write bounded overrides for that private profile's Owl. The verified editor receives the merged public-plus-camp catalogue; ordinary visitors receive only the public catalogue. Saving Camp Hexadecibel updates the V4 tier and synchronizes the Owl to its attached Hexlace while preserving seed and number. The route keeps its old name for storage compatibility.
- `POST /camp/pairings` `{role: "member"|"admin"}` + admin bearer → creates a hashed, one-use, 10-minute access-only QR pass; `POST /camp/pairings/redeem` registers the receiving device's generated bearer key without touching its existing Hexlace identity, local profile data, or NFC tag. A retry from the same device key is idempotent; a second device is rejected. The normal UI exposes no access controls, and the admin UI intentionally does not offer copy/share/NFC actions for these passes.
- `GET /camp/access` + bearer → current active role. `POST /camp/access/revoke` requires admin and revokes a Hexlace's grant/device credentials. `POST /camp/bootstrap` requires the one-time Worker secret plus a valid Hexlace write key; see `CAMP-ACCESS.md`.
...
- Terminology is strict: **Hexlace** means the physical NFC tag; **Hex Owl** means the generated identity image. Every current Owl stores `version:4` plus a `public` or `camp-hexadecibel` tier. Legacy V1/V2 identities lazily normalize to V4/public and legacy V3 identities normalize to V4/camp without changing seed, number, mint time, season, or visible art. The frozen V2 roll grammar supplies Common/Rare/Legendary; the frozen V3 roll grammar supplies Camp Hexadecibel's six UV palettes, mandatory Vortex, hero floor/cap, support cap, and 12-point budget. These are internal appearance grammars, not separate user populations. The public catalogue excludes the camp rarity and camp-only values; a verified member/admin editor receives the merged catalogue. Shared or tapped camp Owls still render and collect for anyone, and camp-tier Owls cannot trade.
- V2 reconciliation notes: live v59 has 36 public palettes (12 families with Daylight, Electric, and Midnight variants), so v2 preserves all 36 rather than silently dropping the Midnight variants to match the package's contradictory 24-palette arithmetic. Brow Echo's source highlight is a filled brow subpath rather than a stroke; v2 applies the specified full shadow opacity but keeps the existing `.94` highlight band because the package supplies no thinner replacement geometry.
...
Unreleased: v71 restores the hexagonal schedule timeline and trailing Save controls, makes overlap badges operable, separates the non-sticky live card from day-first schedule controls, shows the return-to-now button as soon as the live card leaves view, makes My Set List collapsible, moves Username and Share into its body, replaces the planner's live-summary bars with current/up-next row states, removes Clear all, aligns Hexlace status with its title, and removes the main schedule's gold Up Next row highlight.

Latest: v71 refines the v70 layout from field feedback: the schedule regains its hexagonal timeline and right-side Save controls, overlap buttons can be opened directly, the live card no longer sticks, the day selector precedes the stage selector, and the larger stage mark sits immediately above the schedule. v70 integrates the approved schedule design review while retaining pings, comparison, release, camp controls, and camp-only Owl customization; the standalone Owl card and user-facing trade flow are removed, with all Owl information now living in the Hexadex. v69 unifies all current Owls under the V4 identity contract, keeps public and Camp Hexadecibel as rarity tiers, preserves every legacy appearance during lazy migration, and reveals camp choices only in the verified member/admin editor. v68 adds the provenance-only Camp Hexadecibel Owl V3 renderer and camp-claim promotion, prevents V3 Owls from entering physical-tag trades, reports camp customizer budget and grammar issues, and blocks a new sharing name until at least one set is saved in both the app and Worker. v67 makes camp member/admin Owl editing fully freestyle: every enabled aura, brow, ring, eye, and other trait applies without rarity budgets, weighted rolls, mandatory hero/ring grammar, or combination caps, while ordinary generated Owls retain the V2 rules. v66 restores ordinary no-camp claimable tags and adds the camp Hex Owl customizer for members and admins, with dropdowns sourced from the current renderer catalogue, original-versus-custom preview, validated combinations, and private-profile saves. v65 restores the ordinary visitor interface by removing all public camp-access controls and adds in-person, one-use admin-created QR passes for either member or admin access without changing the recipient's profile, saved data, Owl, or NFC tag. v64 adds accountless camp member/admin Hexlace access, server-enforced Admin options, hashed and revocable device credentials, and a schema-driven admin Hex Owl override framework. See `CAMP-ACCESS.md` for bootstrap and issuance. v63 makes friend Hexlace scans refresh and merge in the background, so a harmless collection scan never presents the owner-conflict choice. v62 adds cancellation-aware schedule, live-status, My Set List, and Hexlace rendering; marks Whethan and Rusko cancelled while preserving their saved-set identities; and restores the Sunday Grove Circus Acts Insomniacs slot. v61 upgrades Hex Owl and Hexadex discovery with tappable Owl details, rarity and trait presentation, an own-Owl Hexadex slot, clearer first-open guidance, a promoted avatar opener, simplified Friends counts, and progressively disclosed My Hexlace helper copy. v60 adds the version-gated Hex Owl V2 trait/art grammar, 50/30/20 rarity composition, deterministic prism shuffles, new eye/beak/brow/marking/aura treatments, and V2 assignment while preserving byte-identical V1 rendering. v59 replaces visible cross-SVG Owl sprite references with direct vector paths, removes the playground backdrop-blur compositor layer, and displays both renderer and release versions for Android diagnostics. v58 documents the live Hex Owl playground, fixes Chrome option-menu contrast, and removes the Android Chrome external-image mask raster path. v57 replaces pinpoint overlays with palette-linked eye-well fills, disables the first accessory concepts, aligns every decorative beak to the supplied Owl apex, and repairs visible Rare/Legendary auras. v56 replaced the pre-festival Hex Owl art grammar with frozen palettes, rarity budgets, measured portal geometry, safe-zone traits, split lasers, and a deterministic playground/test suite. v55 added deterministic numbered Hex Owls, private multi-year Hexadex collection, tap-specific physical URLs, release/reclaim continuity, and mandatory Owl-with-Hexlace trade semantics. v54 added the minimal one-day-at-a-time plan comparison flow. v53 hid unavailable installed-app connection UI and simplified footer copy. v52 redesigned My Hexlace and added release plus reciprocal confirmed physical trades.
