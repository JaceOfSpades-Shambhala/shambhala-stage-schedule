...
test("release assets and service-worker precache use one version and include both fonts", async () => {
  ...
  assert.deepEqual([...referencedVersions], ["76"], "Every release asset query must use exactly v76.");
  assert.equal(html.match(/<!--\s*v(\d+)\s*-->/)?.[1], "76", "The Pages release marker must be v76.");
  assert.equal(serviceWorker.match(/stage-schedule-v(\d+)/)?.[1], "76", "The service-worker cache must be v76.");
  assert.match(readme, /authoritative deployed version[^\n]*\bv76\b/i);
  assert.match(handoff, /current release \*\*v76\*\*/i);
  assert.match(handoff, /release bumps ONE version number everywhere \(v76 at the time of writing\)/);
  assert.match(serviceWorker, /InterVariable\.woff2\?v=76/);
  assert.match(serviceWorker, /InterVariable-Italic\.woff2\?v=76/);
  assert.match(css, /InterVariable\.woff2\?v=76/);
  assert.match(css, /InterVariable-Italic\.woff2\?v=76/);
  assert.match(serviceWorker, /schedule-metadata\.js\?v=76/);
  assert.match(serviceWorker, /undo\.js\?v=76/);
  assert.match(serviceWorker, /camp-access\.js\?v=76/);
  assert.match(serviceWorker, /hexlace-compare\.js\?v=76/);
  assert.match(serviceWorker, /hex-owl\.js\?v=76/);
  assert.match(serviceWorker, /hex-owl-base\.svg\?v=76/);
  assert.match(serviceWorker, /hex-owl-playground\.html/);
  assert.match(serviceWorker, /hexadex\.js\?v=76/);
  assert.match(hexOwl, /hex-owl-base\.svg\?v=76/);
  assert.match(playground, /hex-owl\.js\?v=76/);
});
