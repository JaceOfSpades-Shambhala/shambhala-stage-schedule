...
test("release assets and service-worker precache use one version and include both fonts", async () => {
  ...
  assert.deepEqual([...referencedVersions], ["64"], "Every release asset query must use exactly v64.");
  assert.equal(html.match(/<!--\s*v(\d+)\s*-->/)?.[1], "64", "The Pages release marker must be v64.");
  assert.equal(serviceWorker.match(/stage-schedule-v(\d+)/)?.[1], "64", "The service-worker cache must be v64.");
  assert.match(readme, /authoritative deployed version[^\n]*\bv64\b/i);
  assert.match(handoff, /current release \*\*v64\*\*/i);
  assert.match(handoff, /release bumps ONE version number everywhere \(v64 at the time of writing\)/);
  assert.match(serviceWorker, /InterVariable\.woff2\?v=64/);
  assert.match(serviceWorker, /InterVariable-Italic\.woff2\?v=64/);
  assert.match(css, /InterVariable\.woff2\?v=64/);
  assert.match(css, /InterVariable-Italic\.woff2\?v=64/);
  assert.match(serviceWorker, /schedule-metadata\.js\?v=64/);
  assert.match(serviceWorker, /undo\.js\?v=64/);
  assert.match(serviceWorker, /camp-access\.js\?v=64/);
  assert.match(serviceWorker, /hexlace-compare\.js\?v=64/);
  assert.match(serviceWorker, /hex-owl\.js\?v=64/);
  assert.match(serviceWorker, /hex-owl-base\.svg\?v=64/);
  assert.match(serviceWorker, /hex-owl-playground\.html/);
  assert.match(serviceWorker, /hexadex\.js\?v=64/);
  assert.match(hexOwl, /hex-owl-base\.svg\?v=64/);
  assert.match(playground, /hex-owl\.js\?v=64/);
});
