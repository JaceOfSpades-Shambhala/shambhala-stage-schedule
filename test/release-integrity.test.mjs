...
test("release assets and service-worker precache use one version and include both fonts", async () => {
  ...
  assert.deepEqual([...referencedVersions], ["67"], "Every release asset query must use exactly v67.");
  assert.equal(html.match(/<!--\s*v(\d+)\s*-->/)?.[1], "67", "The Pages release marker must be v67.");
  assert.equal(serviceWorker.match(/stage-schedule-v(\d+)/)?.[1], "67", "The service-worker cache must be v67.");
  assert.match(readme, /authoritative deployed version[^\n]*\bv67\b/i);
  assert.match(handoff, /current release \*\*v67\*\*/i);
  assert.match(handoff, /release bumps ONE version number everywhere \(v67 at the time of writing\)/);
  assert.match(serviceWorker, /InterVariable\.woff2\?v=67/);
  assert.match(serviceWorker, /InterVariable-Italic\.woff2\?v=67/);
  assert.match(css, /InterVariable\.woff2\?v=67/);
  assert.match(css, /InterVariable-Italic\.woff2\?v=67/);
  assert.match(serviceWorker, /schedule-metadata\.js\?v=67/);
  assert.match(serviceWorker, /undo\.js\?v=67/);
  assert.match(serviceWorker, /camp-access\.js\?v=67/);
  assert.match(serviceWorker, /hexlace-compare\.js\?v=67/);
  assert.match(serviceWorker, /hex-owl\.js\?v=67/);
  assert.match(serviceWorker, /hex-owl-base\.svg\?v=67/);
  assert.match(serviceWorker, /hex-owl-playground\.html/);
  assert.match(serviceWorker, /hexadex\.js\?v=67/);
  assert.match(hexOwl, /hex-owl-base\.svg\?v=67/);
  assert.match(playground, /hex-owl\.js\?v=67/);
});
