...
The editor shows the immutable original Owl beside a live preview. **Use original traits** clears every override; it does not change the Owl seed or number. Camp rendering is freestyle: every enabled selection applies directly, without rarity budgets, weighted rolls, mandatory hero/ring grammar, treatment caps, or conflict repairs. Leaving a dropdown on **Original** keeps that seeded trait.

Saved choices are bounded simple values stored on the existing private Hex Owl profile, republished with that Owl, and rendered through the renderer's camp-only freestyle override path. Unknown or disabled catalogue values are still rejected. The existing `/owl-admin-traits` route name remains for storage compatibility even though both camp roles may now use it. The Worker accepts at most 24 values with bounded keys and scalar string, number, boolean, or null values.
