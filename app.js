(() => {
  ...
  function setStageMarkArt(stageLabel) {
    ...
    art.src = `stage-names/${appState.stage}.png?v=54`;
    art.alt = stageLabel;
    art.width = 150;
    art.height = 64;
  ...
  const SCHEDULE_ASSET = "schedule-metadata.js?v=54";
  ...
  window.setTimeout(() => {
    STAGES.forEach(stage => {
      ...
      const img = new Image();
      img.src = `stage-names/${stage.id}.png?v=54`;
    });
  }, 1500);
  ...
  if ("serviceWorker" in navigator) window.addEventListener("load", () => {
    navigator.serviceWorker.register("sw.js?v=54").then(registerPeriodicSync).catch(() => {});
  });
})();
